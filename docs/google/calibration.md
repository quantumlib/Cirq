...
# TODO(#2720): this PR refactors some of these functions...

# gets the latest calibration metrics
latest_calibration = engine.get_latest_calibration()
...
(those generated by the Pauli gates {±X,±Y,±Z}) then applying the inverse of the
expected final state.  The result of this sequence should always be identity
(|0> state).  The final error is measured and compared against this state to
produce the total error.  This error is calculated for one qubit at a time while
all other qubits on the device are idle (isolated).

The purity error is a measure of the incoherent error (as opposed to
coherent, or control, error) and is a measure of the
[Purity](https://en.wikipedia.org/wiki/Purity_(quantum_mechanics)) of the
quantum state after the randomized benchmark circuit has been applied.
