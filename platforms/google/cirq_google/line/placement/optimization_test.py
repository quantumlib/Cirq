...
def test_anneal_minimize_improves_when_better():
    assert optimization.anneal_minimize(
        'initial', lambda s: 1.0 if s == 'initial' else 0.0, lambda s: 'better',
        lambda: 1.0, 1.0, 0.5, 0.5, 1) == 'better'


def test_anneal_minimize_keeps_when_worse_and_discarded():
    assert optimization.anneal_minimize(
        'initial', lambda s: 0.0 if s == 'initial' else 1.0, lambda s: 'better',
        lambda: 0.9, 1.0, 0.5, 0.5, 1) == 'initial'


def test_anneal_minimize_raises_when_wrong_cooling_factor():
    with pytest.raises(ValueError):
        optimization.anneal_minimize('initial', lambda s: 1.0 if s == 'initial'
                                     else 0.0, lambda s: 'better', lambda: 1.0,
                                     1.0, 0.5, 2.0, 1)
