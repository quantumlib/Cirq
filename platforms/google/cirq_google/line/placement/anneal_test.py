...
def test_search_calls_anneal_minimize(anneal_minimize):
    ...
    anneal_minimize.assert_called_once_with(mock.ANY,
                                            mock.ANY,
                                            mock.ANY,
                                            mock.ANY,
                                            trace_func=mock.ANY)
...
def test_quadratic_sum_cost_calculates_quadratic_cost():
    ...
    def calculate_cost(seqs: List[List[GridQubit]]):
        ...
        for seq in seqs:
            qubits += seq
        return AnnealSequenceSearch(_create_device(qubits),
                                    seed=0xF00D0003)._quadratic_sum_cost(
                                        (seqs, set()))
...
def test_force_edge_active_creates_valid_solution_single_sequence():
    ...
    assert search._force_edge_active(
        ...
        (q30, q31), lambda: True) == [[q20, q10, q00, q01, q11, q21, q31, q30]]
    ...
    assert search._force_edge_active(
        ...
        (q30, q31), lambda: True) == [[q21, q11, q01, q00, q10, q20, q30, q31]]
    ...
    assert search._force_edge_active(
        [[q20, q10, q00, q01, q11, q21, q31], [q30]],
        (q20, q21), lambda: next(samples)) == [[q30],
                                               [
                                                   q31, q21, q20, q10, q00, q01,
                                                   q11
                                               ]]
...
def test_create_initial_solution_creates_valid_solution():
    ...
    def check_chip(qubits: List[GridQubit]):
        _verify_valid_state(
            qubits,
            AnnealSequenceSearch(_create_device(qubits),
