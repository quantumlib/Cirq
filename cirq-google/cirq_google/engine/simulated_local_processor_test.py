...
def test_calibrations():
    ...
    assert proc.list_calibrations(
        earliest_timestamp_seconds=5000, latest_timestamp_seconds=15000
    ) == [cal1]
    assert proc.list_calibrations(
        earliest_timestamp_seconds=15000, latest_timestamp_seconds=25000
    ) == [cal2]
    assert proc.list_calibrations(
        earliest_timestamp_seconds=future - 1, latest_timestamp_seconds=future + 1
    ) == [cal3]
    cal_list = proc.list_calibrations(latest_timestamp_seconds=25000)
    assert len(cal_list) == 2
    assert cal1 in cal_list
    assert cal2 in cal_list
    cal_list = proc.list_calibrations(earliest_timestamp_seconds=15000)
    assert len(cal_list) == 2
    assert cal2 in cal_list
