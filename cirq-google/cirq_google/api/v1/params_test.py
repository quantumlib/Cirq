...
def test_gen_sweep_linspace():
    sweep = params_pb2.SingleSweep(
        parameter_key='foo',
        linspace=params_pb2.Linspace(first_point=0, last_point=10, num_points=11),
    )
    out = params._sweep_from_single_param_sweep_proto(sweep)
...
def test_gen_param_sweep_zip():
    sweep = params_pb2.ZipSweep(
        sweeps=[
            params_pb2.SingleSweep(parameter_key='foo', points=params_pb2.Points(points=[1, 2, 3])),
            params_pb2.SingleSweep(parameter_key='bar', points=params_pb2.Points(points=[4, 5])),
        ]
    )
...
def test_sweep_from_proto_missing_type():
    ps = params_pb2.ParameterSweep(
        sweep=params_pb2.ProductSweep(
            factors=[params_pb2.ZipSweep(sweeps=[params_pb2.SingleSweep(parameter_key='foo')])]
        )
    )
            ...
            factors=[
                params_pb2.ZipSweep(
                    sweeps=[
                        ...
                        params_pb2.SingleSweep(
                            parameter_key='12', points=params_pb2.Points(points=range(7))
                        ),
                    ]
                ),
                params_pb2.ZipSweep(
                    sweeps=[
                        ...
                        params_pb2.SingleSweep(
                            parameter_key='22', points=params_pb2.Points(points=range(13))
                        ),
                    ]
                ),
            ]
...
def test_param_sweep_size_no_sweeps():
    ps = params_pb2.ParameterSweep(
        sweep=params_pb2.ProductSweep(factors=[params_pb2.ZipSweep(), params_pb2.ZipSweep()])
    )
...
def example_sweeps():
    ...
    empty_zip = params_pb2.ParameterSweep(
        sweep=params_pb2.ProductSweep(factors=[params_pb2.ZipSweep(), params_pb2.ZipSweep()])
    )
    full_sweep = params_pb2.ParameterSweep(
        sweep=params_pb2.ProductSweep(
            factors=[
                params_pb2.ZipSweep(
                    sweeps=[
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (cirq.Linspace('a', 0, 10, 25), cirq.Product(cirq.Zip(cirq.Linspace('a', 0, 10, 25)))),
        (cirq.Points('a', [1, 2, 3]), cirq.Product(cirq.Zip(cirq.Points('a', [1, 2, 3])))),
        (
            cirq.Zip(cirq.Linspace('a', 0, 1, 5), cirq.Points('b', [1, 2, 3])),
            cirq.Product(cirq.Zip(cirq.Linspace('a', 0, 1, 5), cirq.Points('b', [1, 2, 3]))),
        ),
        (
            cirq.Product(cirq.Linspace('a', 0, 1, 5), cirq.Points('b', [1, 2, 3])),
            cirq.Product(
                cirq.Zip(cirq.Linspace('a', 0, 1, 5)), cirq.Zip(cirq.Points('b', [1, 2, 3]))
            ),
        ),
        (
            cirq.Product(
        ...
        (
            cirq.Product(
                cirq.Zip(cirq.Linspace('a', 0, 1, 5), cirq.Points('b', [1, 2, 3])),
                cirq.Zip(cirq.Linspace('c', 0, 1, 8), cirq.Points('d', [1, 0.5, 0.25, 0.125])),
            ),
            cirq.Product(
                cirq.Zip(cirq.Linspace('a', 0, 1, 5), cirq.Points('b', [1, 2, 3])),
                cirq.Zip(cirq.Linspace('c', 0, 1, 8), cirq.Points('d', [1, 0.5, 0.25, 0.125])),
...
@pytest.mark.parametrize(
    'bad_sweep',
    [
        cirq.Zip(cirq.Product(cirq.Linspace('a', 0, 10, 25), cirq.Linspace('b', 0, 10, 25))),
