...
def _random_pauli_string(qubits, rs, coefficients=False):
    ps = cirq.PauliString({q: p} for q, p in zip(qubits, rs.choice([cirq.X, cirq.Y, cirq.Z, cirq.I],
                                                                   size=len(qubits))))
    if coefficients:
...
def test_sandwich_operator_expect_val():
    ...
    for _ in range(10):  # try a bunch of different ones
        circuit = cirq.testing.random_circuit(qubits=qubits, n_moments=10,
                                              op_density=0.8, random_state=rs)
...
def test_tensor_expectation_value():
    ...
    for _ in range(10):
        for n_qubit in [2, 7]:
            ...
            for depth in [10, 20]:
                circuit = cirq.testing.random_circuit(qubits=qubits, n_moments=depth,
                                                      op_density=0.8, random_state=rs)
                operator = _random_pauli_string(qubits, rs, coefficients=True)
                eval_tn = ccq.tensor_expectation_value(circuit, operator)

                wfn = cirq.Simulator().simulate(circuit)
                eval_normal = operator.expectation_from_wavefunction(wfn.final_state,
                                                                     wfn.qubit_map)
