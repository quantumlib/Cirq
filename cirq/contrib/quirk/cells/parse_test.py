...
def test_parse_complex_expression_cases_from_quirk():
    ...
    assert parse_complex("(1/3)+2i") == 1/3 + 2j
    np.testing.assert_allclose(
        parse_complex("1/(3+2i)"),
        1 / (3 + 2j),
        atol=1e-8)
    np.testing.assert_allclose(
        parse_complex("1/sqrt(3+2i)"),
        1 / ((3 + 2j)**0.5),
        atol=1e-8)
    ...
    np.testing.assert_allclose(
        parse_complex("cos(pi/4) + i sin(pi/4)"),
        np.sqrt(0.5) + 1j*np.sqrt(0.5),
        atol=1e-8)
    np.testing.assert_allclose(
        parse_complex("cos(pi) + i (sin pi)"),
        -1,
        atol=1e-8)
    np.testing.assert_allclose(
        parse_complex("e^(pi i)"),
        -1,
        atol=1e-8)
    np.testing.assert_allclose(
        parse_complex("exp(ln(2))"),
        2,
        atol=1e-8)
    np.testing.assert_allclose(
        parse_complex("sin(arcsin(0.5))"),
        0.5,
        atol=1e-8)
    np.testing.assert_allclose(
        parse_complex("cos(arccos(0.5))"),
        0.5,
        atol=1e-8)
    np.testing.assert_allclose(
        parse_complex("sin(asin(0.5))"),
        0.5,
        atol=1e-8)
    np.testing.assert_allclose(
        parse_complex("cos(acos(0.5))"),
        0.5,
        atol=1e-8)
