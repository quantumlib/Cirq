...
def test_controlled_gate():
    ...
    # Unknown sub-gate.
    circuit = cirq.Circuit.from_ops(cirq.ControlledGate(MysteryGate()).on(a, b))
    assert_links_to(circuit,
                    """
...
def test_fredkin():
    ...
    circuit = cirq.Circuit.from_ops(cirq.FREDKIN(a, b, c))
    assert_links_to(circuit,
                    """
    ...
    circuit = cirq.Circuit.from_ops(cirq.CSWAP(a, b, c), cirq.CSWAP(x, y, z))
    assert_links_to(circuit,
                    """
        http://algassert.com/quirk#circuit={"cols":[
            ["•","Swap","Swap"],
...
def test_ccz():
    ...
    # Raw.
    circuit = cirq.Circuit.from_ops(cirq.CCZ(a, b, c))
    assert_links_to(circuit,
                    """
        http://algassert.com/quirk#circuit={"cols":[["•","•","Z"]]}
    """,
    ...
    # Symbolic exponent.
    circuit = cirq.Circuit.from_ops(cirq.CCZ(a, b, c)**sympy.Symbol('t'))
    assert_links_to(circuit,
                    """
    ...
    # Unknown exponent.
    circuit = cirq.Circuit.from_ops(cirq.CCZ(a, b, c)**0.01)
    assert_links_to(circuit,
                    """
        http://algassert.com/quirk#circuit={"cols":[
            ["UNKNOWN","UNKNOWN","UNKNOWN"]
    ...
    # With exponent. Doesn't merge with other operation.
    circuit = cirq.Circuit.from_ops(cirq.CCZ(a, b, c)**0.5, cirq.H(d))
    assert_links_to(circuit,
                    """
        http://algassert.com/quirk#circuit={"cols":[
            ["•","•","Z^½"],[1,1,1,"H"]]}
