...
def skip_and_wrap_around(items: Sequence[TItem]) -> Tuple[TItem, ...]:
    ...
    positions = {
        p: i for i, p in enumerate(
            itertools.chain(range(0, n_items, 2), reversed(range(1, n_items,
                                                                 2))))
    }
    return tuple(items[positions[i]] for i in range(n_items))


def cubic_acquaintance_strategy(qubits: Iterable[ops.Qid],
    ...
    for separation in range(1, max_separation):
        ...
        permutation = {
            i: new_index_order.index(j) for i, j in enumerate(index_order)
        }
