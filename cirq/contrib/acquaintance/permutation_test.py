...
def test_swap_permutation_gate():
    ...
    no_decomp = lambda op: (op.gate is not None and op.gate == cirq.CZ)
    expander = cirq.ExpandComposite(no_decomp=no_decomp)
...
@pytest.mark.parametrize(
    ...
    [
        cirq.Circuit(
            ...
            for qubit_pair in [random.sample(cirq.LineQubit.range(10), 2) for _ in range(20)]
        )
        for _ in range(4)
    ],
)
...
def test_uses_consistent_swap_gate():
    ...
    circuit = cirq.Circuit(
        [cca.SwapPermutationGate(cirq.CZ)(a, b), cca.SwapPermutationGate(cirq.CZ)(a, b)]
    )
    assert cca.uses_consistent_swap_gate(circuit, cirq.CZ)
    assert not cca.uses_consistent_swap_gate(circuit, cirq.SWAP)
    circuit = cirq.Circuit(
        [cca.SwapPermutationGate()(a, b), cca.SwapPermutationGate(cirq.CZ)(a, b)]
    )
    assert not cca.uses_consistent_swap_gate(circuit, cirq.SWAP)
    assert not cca.uses_consistent_swap_gate(circuit, cirq.CZ)
