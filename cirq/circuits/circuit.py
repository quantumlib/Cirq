...
class AbstractCircuit(metaclass=abc.ABCMeta):
    ...
    # Overloads for common double-underscore methods.
    ...
    def __repr__(self) -> str:
        ...
        if not self.moments:
            return f'cirq.AbstractCircuit(device={self.device!r})'
        ...
        return f'cirq.AbstractCircuit({moment_str}, device={self.device!r})'
    ...
    def next_moment_operating_on(self,
        ...
        return self._first_moment_operating_on(
            qubits, range(start_moment_index,
                          start_moment_index + max_distance))

    def next_moments_operating_on(
            self,
            qubits: Iterable['cirq.Qid'],
            start_moment_index: int = 0) -> Dict['cirq.Qid', int]:
        ...
        *,
        ...
        while queue:
            ...
            for q in cur_op.qubits:
                if (q in start_frontier and cur_moment >= start_frontier[q] and
    ...
    def are_all_matches_terminal(self, predicate: Callable[['cirq.Operation'],
                                                           bool]):
...
class Circuit(AbstractCircuit):
    ...
    def _pick_or_create_inserted_op_moment_index(
            self, splitter_index: int, op: 'cirq.Operation',
            strategy: 'cirq.InsertStrategy') -> int:
    ...
    def _push_frontier(
            ...
            update_qubits: Iterable['cirq.Qid'] = None) -> Tuple[int, int]:
    ...
    def insert_at_frontier(
            ...
            start: int,
            frontier: Dict['cirq.Qid', int] = None) -> Dict['cirq.Qid', int]:
    ...
    def batch_replace(
        self, replacements: Iterable[Tuple[int, 'cirq.Operation',
                                           'cirq.Operation']]
    ) -> None:
    ...
    def batch_insert_into(
            self, insert_intos: Iterable[Tuple[int, 'cirq.OP_TREE']]) -> None:
