...
t1 = cirq.TrialResult(params=cirq.ParamResolver({'a': 2}),
                      repetitions=2,
...
def test_str():
    result1 = cirq.TrialResult(params=cirq.ParamResolver({}),
                               repetitions=5,
                               measurements={
                                   'ab':
    ...
    result2 = cirq.TrialResult(params=cirq.ParamResolver({}),
                               repetitions=5,
                               measurements={
                                   'ab':
...
def test_trial_result_equality():
    ...
    et.add_equality_group(
        cirq.SweepTrialResult([t2, t3]), cirq.SweepTrialResult([t3, t2]),
        cirq.SweepTrialResult([t1, t2, t3]).slice_where_params_match({'b': 3}))
    et.add_equality_group(
        cirq.SweepTrialResult([t1, t3]), cirq.SweepTrialResult([t3, t1]),
        cirq.SweepTrialResult([t1, t2, t3]).slice_where_params_match({'a': 2}))
    ...
    et.add_equality_group(
        str([t1, t3]),
        str(
            cirq.SweepTrialResult([t1, t2,
                                   t3]).trials_where_params_match({'a': 2})))
