...
def test_flattener_repr():
    assert repr(cirq.ParamFlattener({'a':
                                     1})) == ("cirq.ParamFlattener({a: 1})")
    assert repr(cirq.ParamFlattener(
        {'a': 1},
        'abcdef')) == ("cirq.ParamFlattener({a: 1}, new_param_names='abcdef')")
...
def test_transformed_sweep():
    a = sympy.Symbol('a')
    sweep = cirq.Linspace('a', start=0, stop=3, length=4)
    flattener = cirq.ParamFlattener({a / 4: 'x0', 1 - a / 4: 'x1'})
...
def test_transformed_sweep_equality():
    ...
    flattener2 = cirq.ParamFlattener({a / 4: 'x0', 1 - a / 4: 'x1'})

    sweep3 = cirq.Linspace(a, start=0, stop=3, length=20)
    flattener3 = cirq.ParamFlattener({a / 20: 'x0', 1 - a / 20: 'x1'})

    et = cirq.testing.EqualsTester()
    et.make_equality_group(lambda: flattener.transform_sweep(
        sweep), lambda: flattener.transform_sweep(sweep2), lambda: flattener2.
