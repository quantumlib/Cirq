...
def test_clifford_trial_result_repr():
    q0 = cirq.LineQubit(0)
    final_simulator_state = cirq.CliffordState(
        qubit_map={q0: 0})
...
def test_clifford_ch_form_repr_full():
    (q0,q1) = (cirq.LineQubit(0),cirq.LineQubit(1))
    state = cirq.CliffordState(qubit_map={q0: 0,q1: 1})
...
def test_clifford_circuit():
    ...
    np.testing.assert_almost_equal(clifford_simulator.simulate(circuit).final_state.wave_function(),
        wave_function_simulator.simulate(circuit).final_state)

def test_clifford_circuit_2():
    (q0,q1) = (cirq.LineQubit(0),cirq.LineQubit(1))
    circuit = cirq.Circuit()
