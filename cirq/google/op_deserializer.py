...
from typing import Dict, Generic, Iterable, NamedTuple, Optional, Type, TypeVar
...
class GateOpDeserializer(Generic[Gate]):
    ...
    def from_proto(self, proto: Dict) -> ops.GateOperation:
        ...
        qubits = [devices.GridQubit.from_proto_dict(proto['qubits'])]
        args = self._args_from_proto(proto['args'])
    ...
    def _args_from_proto(self, args_proto_dict: Dict
        ...
        for arg in self.args:
            if arg.required and arg.serialized_name not in args_proto_dict:
                raise ValueError('Required arg missing: {}'.format(
                    arg.serialized_name))
            value = None
            ...
            if 'arg_value' in args_proto_dict[arg.serialized_name]:
                ...
                if 'bool_values' in arg_value:
                    value = list(arg_value['bool_values']['values'])
                if 'string_value' in arg_value:
                    value = str(arg_value['str_value'])
            ...
            if value is not None:
                return_args[arg.constructor_name] = value
        return return_args
