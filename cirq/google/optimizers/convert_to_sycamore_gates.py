...
class ConvertToSycamoreGates(circuits.PointOptimizer):
    ...
    def known_two_q_operations_to_sycamore_operations(
        ...
        if isinstance(gate, ops.CNotPowGate):
            ...
            if tabulation:
                new_ops = []
                result = tabulation.compile_two_qubit_gate(
                    protocols.unitary(op))
                local_gates = result.local_unitaries
                for i, gate_pairs in enumerate(local_gates):
                    new_ops.extend([
                        term.on(qubit_a) for term in
                        optimizers.single_qubit_matrix_to_gates(gate_pairs[0])
                    ])
                    new_ops.extend([
                        term.on(qubit_b) for term in
                        optimizers.single_qubit_matrix_to_gates(gate_pairs[1])
                    ])
