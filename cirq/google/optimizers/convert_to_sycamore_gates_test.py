...
def test_known_two_q_operations_to_sycamore_operations_cz():
    ...
    true_op_tree = cirq.Circuit(cgoc.decompose_cz_into_syc(
        qubits[0], qubits[1]))
...
def test_known_two_q_operations_to_sycamore_operations_cphase():
    ...
    for phi in np.linspace(0, 2 * np.pi, 30):
        ...
        test_op_tree = cirq.Circuit(operation)
        cgoc.ConvertToSycamoreGates().optimize_circuit(test_op_tree)
        true_op_tree = cirq.Circuit(cgoc.cphase(phi, qubits[0], qubits[1]))
        _assert_equivalent_op_tree(test_op_tree, true_op_tree)
...
def test_known_two_q_operations_to_sycamore_operations_zz():
    ...
    for theta in np.linspace(0, 2 * np.pi, 30):
        ...
        test_op_tree = cirq.Circuit(operation)
        cgoc.ConvertToSycamoreGates().optimize_circuit(test_op_tree)
        true_op_tree = cirq.Circuit(cgoc.rzz(theta, qubits[0], qubits[1]))
        _assert_equivalent_op_tree(test_op_tree, true_op_tree)
...
def test_known_two_q_operations_to_sycamore_operations_swap():
    ...
    true_op_tree = cirq.Circuit(
        cgoc.decompose_swap_into_syc(qubits[0], qubits[1]))
    _assert_equivalent_op_tree(test_op_tree, true_op_tree)
...
def test_known_two_q_operations_to_sycamore_operations_iswap():
    ...
    cgoc.ConvertToSycamoreGates().optimize_circuit(test_op_tree)
    true_op_tree = cirq.Circuit(
        cgoc.decompose_iswap_into_syc(qubits[0], qubits[1]))
    _assert_equivalent_op_tree(test_op_tree, true_op_tree)
...
def test_sycamorecz():
    ...
    sycamore_cz = cirq.Circuit(cgoc.decompose_cz_into_syc(qubits[0], qubits[1]))
...
def test_operator_decomp():
    ...
    op = np.kron(G2, G1)
    coeff_vec = cgoc.operator_decomp(op)
...
def test_cz_reconstruction():
    cz = np.diag([1, 1, 1, -1])
    cz_decomp = cgoc.operator_decomp(cz)
...
def test_schmidt_decomp():
    ...
    A[0, 1] = 1j * c1 * s2
    A[1, 0] = 1j * s1 * c2
    g_decomp = cgoc.operator_decomp(np.kron(G2, G1)).reshape((4, 4), order='F')
    ...
    b_decomp = cgoc.operator_decomp(Bop)
    ...
    c_decomp = cgoc.operator_decomp(Cop)
    ...
    Dop = zz @ Bop @ zz
    d_decomp = cgoc.operator_decomp(Dop)
    ...
    bisycamore_decomp = cgoc.operator_decomp(bisycamore)
...
def test_decompose_cphase():
    ...
    for theta in thetas:
        ...
        u_decomp = cgoc.operator_decomp(u)
        ...
        def optimization_at(self, circuit, index, op):
            if cirq.op_gate_of_type(op, cirq.ZZPowGate):
                return cirq.PointOptimizationSummary(
                    ...
                    new_operations=cgoc.rzz(theta=np.pi * op.gate.exponent / 2,
                                            q0=op.qubits[0],
                                            q1=op.qubits[1]))
...
def test_zztheta_zzpow_unsorted_qubits():
    ...
    syc_circuit = cirq.Circuit(
        cgoc.rzz(theta=np.pi * exponent / 2, q0=qubits[0], q1=qubits[1]))
...
def test_swap_zztheta():
    ...
    for theta in np.linspace(0, 2 * np.pi, 10):
        ...
        actual_circuit = cirq.Circuit(cgoc.swap_rzz(theta, a, b))
...
def test_cphase():
    ...
    for phi in np.linspace(0, 2 * np.pi, 100):
        ...
        test_cphase = cirq.unitary(
            cirq.Circuit(
                cgoc.cphase(phi, cirq.NamedQubit('a'), cirq.NamedQubit('b'))))
...
def test_known_two_q_operations_to_sycamore_operations_cnot():
    ...
    decomposed = cirq.Circuit(
        cgoc.known_two_q_operations_to_sycamore_operations(a, b, op))
...
def test_overlap_known_two_q_operations_to_sycamore_operations_cphase():
    qubits = [cirq.NamedQubit('a'), cirq.NamedQubit('b')]
    for phi in np.linspace(0, 2 * np.pi, 30):
        operation = cirq.CZPowGate(exponent=phi / np.pi).on(
            qubits[0], qubits[1])
        test_op_tree = cirq.Circuit(
            cgoc.known_two_q_operations_to_sycamore_operations(
...
def test_overlap_known_two_q_operations_to_sycamore_operations_swap():
    qubits = [cirq.NamedQubit('a'), cirq.NamedQubit('b')]
    operation = cirq.SWAP(qubits[0], qubits[1])
    test_op_tree = cirq.Circuit(
        cgoc.known_two_q_operations_to_sycamore_operations(
    ...
    _assert_equivalent_op_tree(
        test_op_tree, cgoc.decompose_swap_into_syc(qubits[0], qubits[1]))


def test_overlap_known_two_q_operations_to_sycamore_operations_iswap():
    qubits = [cirq.NamedQubit('a'), cirq.NamedQubit('b')]
    operation = cirq.ISWAP(qubits[0], qubits[1])
    test_op_tree = cirq.Circuit(
        cgoc.known_two_q_operations_to_sycamore_operations(
    ...
    _assert_equivalent_op_tree(
        test_op_tree,
        cirq.Circuit(cgoc.decompose_iswap_into_syc(qubits[0], qubits[1])))
