...
def test_eq():
    ...
    eq.make_equality_group(lambda: cirq.PhasedXZPowGate(
        x_exponent=0.25,
        z_exponent=0.5,
        axis_phase_exponent=0.75))

    # Sensitive to each parameter.
    eq.add_equality_group(cirq.PhasedXZPowGate(
        x_exponent=0,
        z_exponent=0.5,
        axis_phase_exponent=0.75))
    eq.add_equality_group(cirq.PhasedXZPowGate(
...
def test_canonicalization():
    def f(x, z, a):
        return cirq.PhasedXZPowGate(x_exponent=x, z_exponent=z, axis_phase_exponent=a)
...
def test_from_matrix():
    ...
    assert cirq.approx_eq(
        cirq.PhasedXZPowGate.from_matrix(cirq.unitary(cirq.X**0.1)),
        cirq.PhasedXZPowGate(
            x_exponent=0.1,
            z_exponent=0,
            axis_phase_exponent=0),
        atol=1e-8)
    assert cirq.approx_eq(
        ...
        atol=1e-8)
    assert cirq.approx_eq(
        cirq.PhasedXZPowGate.from_matrix(cirq.unitary(cirq.Y**0.1)),
        cirq.PhasedXZPowGate(
            x_exponent=0.1,
            z_exponent=0,
    ...
    assert cirq.approx_eq(
        ...
        atol=1e-8)
    assert cirq.approx_eq(
        cirq.PhasedXZPowGate.from_matrix(cirq.unitary(cirq.Z**-0.1)),
        cirq.PhasedXZPowGate(
            x_exponent=0,
            z_exponent=-0.1,
            axis_phase_exponent=0),
    ...
    # Pauli matrices.
    assert cirq.approx_eq(
        cirq.PhasedXZPowGate.from_matrix(np.eye(2)),
        cirq.PhasedXZPowGate(
            x_exponent=0,
            z_exponent=0,
            axis_phase_exponent=0),
        atol=1e-8)
    assert cirq.approx_eq(
        cirq.PhasedXZPowGate.from_matrix(cirq.unitary(cirq.X)),
        cirq.PhasedXZPowGate(
            x_exponent=1,
            z_exponent=0,
            axis_phase_exponent=0),
        atol=1e-8)
    assert cirq.approx_eq(
        cirq.PhasedXZPowGate.from_matrix(cirq.unitary(cirq.Y)),
        cirq.PhasedXZPowGate(
            x_exponent=1,
            z_exponent=0,
            axis_phase_exponent=0.5),
        atol=1e-8)
    assert cirq.approx_eq(
        cirq.PhasedXZPowGate.from_matrix(cirq.unitary(cirq.Z)),
        cirq.PhasedXZPowGate(
            x_exponent=0,
            z_exponent=1,
            axis_phase_exponent=0),
    ...
    b = random.random()
    c = random.random()
    g = cirq.PhasedXZPowGate(x_exponent=a, z_exponent=b, axis_phase_exponent=c)
    assert cirq.approx_eq(
        cirq.PhasedXZPowGate.from_matrix(cirq.unitary(g)),
        g,
        atol=1e-8)
    u = cirq.testing.random_unitary(2)
    cirq.testing.assert_allclose_up_to_global_phase(
        cirq.unitary(cirq.PhasedXZPowGate.from_matrix(u)),
        u,
        atol=1e-8)
...
def test_protocols():
    ...
    # Symbolic.
    t = sympy.Symbol('t')
    g = cirq.PhasedXZPowGate(x_exponent=t, z_exponent=b, axis_phase_exponent=c)
    cirq.testing.assert_implements_consistent_protocols(g)
    g = cirq.PhasedXZPowGate(x_exponent=a, z_exponent=t, axis_phase_exponent=c)
    cirq.testing.assert_implements_consistent_protocols(g)
    g = cirq.PhasedXZPowGate(x_exponent=a, z_exponent=b, axis_phase_exponent=t)
    cirq.testing.assert_implements_consistent_protocols(g)
...
def test_inverse():
    ...
    q = cirq.LineQubit(0)
    g = cirq.PhasedXZPowGate(
        x_exponent=a, z_exponent=b, axis_phase_exponent=c).on(q)
...
def test_parameterized():
    ...
    t = sympy.Symbol('t')
    gt = cirq.PhasedXZPowGate(x_exponent=t, z_exponent=b, axis_phase_exponent=c)
    assert cirq.is_parameterized(gt)
    assert cirq.resolve_parameters(gt, {'t': a}) == g
    gt = cirq.PhasedXZPowGate(x_exponent=a, z_exponent=t, axis_phase_exponent=c)
    assert cirq.is_parameterized(gt)
    assert cirq.resolve_parameters(gt, {'t': b}) == g
    gt = cirq.PhasedXZPowGate(x_exponent=a, z_exponent=b, axis_phase_exponent=t)
    assert cirq.is_parameterized(gt)
    assert cirq.resolve_parameters(gt, {'t': c}) == g


def test_str_diagram():
    g = cirq.PhasedXZPowGate(x_exponent=0.5, z_exponent=0.25, axis_phase_exponent=0.125)

    assert str(g) == "PhXZ(p=0.125,x=0.5,z=0.25)"

    cirq.testing.assert_has_diagram(cirq.Circuit(
        g.on(cirq.LineQubit(0))
    ), """
