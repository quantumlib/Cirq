...
def test_pauli_string_expectation_value_pure_state():
    ...
    np.testing.assert_allclose(
        ...
        0)
    np.testing.assert_allclose(
        z0z3.value_derived_from_density_matrix(density_matrix, qubit_index_map),
        0)
    np.testing.assert_allclose(
        z0x1.value_derived_from_density_matrix(density_matrix, qubit_index_map),
        0)
    np.testing.assert_allclose(
        z1x2.value_derived_from_density_matrix(density_matrix, qubit_index_map),
        -1)
    np.testing.assert_allclose(
        x0z1.value_derived_from_density_matrix(density_matrix, qubit_index_map),
        0)
    np.testing.assert_allclose(
...
def test_approx_pauli_string_expectation_value(measurements, value):
    display = cirq.pauli_string_expectation(cirq.PauliString({}), num_samples=1)
...
def test_with_qubits():
    ...
    assert (cirq.pauli_string_expectation(pauli_string).with_qubits(*new_qubits)
...
def test_pauli_string_expectation_helper():
    ...
    assert (cirq.pauli_string_expectation(
        pauli_string, key='a') == cirq.pauli_string_expectation(pauli_string,
                                                                key='a'))
    assert (cirq.pauli_string_expectation(
        pauli_string, 5, key='a') == cirq.pauli_string_expectation(pauli_string,
                                                                   5,
                                                                   key='a'))
