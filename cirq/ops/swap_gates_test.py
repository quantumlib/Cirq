...
def test_iswap_rotation_unitary(angle_rads):
    actual = cirq.unitary(cirq.iswap(rads=angle_rads))
    c = np.cos(angle_rads)
...
def test_iswap_rotation_has_consistent_protocols(angle_rads):
    cirq.testing.assert_implements_consistent_protocols(
        cirq.iswap(rads=angle_rads), ignoring_global_phase=False)


@pytest.mark.parametrize('angle_rads', (-1, -0.3, 0.2, 1))
def test_deprecated_iswap_rotation(angle_rads):
    assert np.all(
        cirq.unitary(cirq.ISwapRotation(angle_rads)) == cirq.unitary(
            cirq.iswap(rads=angle_rads)))
