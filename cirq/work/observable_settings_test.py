...
from cirq.work.observable_settings import (_max_weight_state,
                                           ...
                                           _hashable_param)
from cirq.work import InitObsSetting, observables_to_settings, MeasurementSpec
...
def test_measurement_spec():
    ...
    setting = InitObsSetting(init_state=cirq.KET_ZERO(q0) * cirq.KET_ZERO(q1),
                             observable=cirq.X(q0) * cirq.Y(q1))
    meas_spec = MeasurementSpec(max_setting=setting,
                                circuit_params={
                                    'beta': 0.123,
                                    'gamma': 0.456,
                                })
    meas_spec2 = MeasurementSpec(max_setting=setting,
                                 circuit_params={
                                     'beta': 0.123,
                                     'gamma': 0.456,
                                 })
