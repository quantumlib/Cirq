...
import time
import uuid
from typing import List, Union, Tuple, Dict, cast, TYPE_CHECKING
...
class AQTSampler(Sampler):
    ...
    def _generate_json(
            self,
            circuit: Circuit,
            param_resolver: ParamResolverOrSimilarType,
    ) -> str:
    ...
    def run_sweep(self,
                  program: Circuit,
                  params: Sweepable,
                  repetitions: int = 1) -> List[TrialResult]:
        ...
        for param_resolver in to_resolvers(params):
            id_str = uuid.uuid1()
