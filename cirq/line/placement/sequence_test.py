...
def test_line_sequence_eq():
    ...
    eq.make_equality_pair(lambda: LineSequence(LineQubit.range(2), []))
    ...
    eq.make_equality_pair(
        lambda: LineSequence(LineQubit.range(2),
                             [XmonQubit(1, 0), XmonQubit(0, 0)]))
...
def test_line_placement_eq():
    ...
    eq.make_equality_pair(
        lambda: LinePlacement(
            [LineSequence(LineQubit.range(2), [XmonQubit(0, 0)])]))
    eq.make_equality_pair(
        lambda: LinePlacement([LineSequence([], [XmonQubit(0, 0)]),
                               LineSequence([], [XmonQubit(0, 1)])]))
    eq.make_equality_pair(
        lambda: LinePlacement([LineSequence([], [XmonQubit(0, 0)]),
                               LineSequence(LineQubit.range(2),
                                            [XmonQubit(0, 1)])]))

# def test_empty_sequence():
#     assert longest_sequence_index([[]]) == []
#
#
