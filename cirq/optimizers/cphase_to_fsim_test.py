...
def test_invalid_qubits():
    with pytest.raises(ValueError):
        cirq.decompose_cphase_into_two_fsim(
            cphase_gate=cirq.CZ, fsim_gate=TestSycamoreGate(), qubits=cirq.LineQubit.range(3)
        )


def test_circuit_structure():
    syc = TestSycamoreGate()
    ...
    for op in ops:
        ...
        if len(op.qubits) == 2:
            ...
            assert isinstance(op.gate, TestSycamoreGate)
...
def test_decomposition_to_sycamore_gate(exponent):
    ...
    assert_decomposition_valid(cphase_gate, TestSycamoreGate())
