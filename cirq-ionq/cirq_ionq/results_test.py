...
def test_qpu_result_to_cirq_result():
    ...
    assert result.to_cirq_result() == cirq.ResultImpl(
        params=cirq.ParamResolver({}), measurements={'x': [[0, 0], [0, 1], [0, 1]]}
    )
    params = cirq.ParamResolver({'a': 0.1})
    assert result.to_cirq_result(params) == cirq.ResultImpl(
...
def test_simulator_result_to_cirq_result():
    ...
    assert result.to_cirq_result(seed=2) == cirq.ResultImpl(
        params=cirq.ParamResolver({}), measurements={'x': [[0, 1], [0, 0], [0, 1]]}
    )
    assert result.to_cirq_result(seed=3) == cirq.ResultImpl(
        params=cirq.ParamResolver({}), measurements={'x': [[0, 1], [0, 1], [0, 1]]}
    )
    params = cirq.ParamResolver({'a': 0.1})
    assert result.to_cirq_result(seed=3, params=params) == cirq.ResultImpl(
        params=params, measurements={'x': [[0, 1], [0, 1], [0, 1]]}
    )
    assert result.to_cirq_result(seed=2, override_repetitions=2) == cirq.ResultImpl(
