...
def test_validate_operation_errors():
    ...
    not_on_device_op = cirq.ParallelGate(cirq.X, 16).on(
    ...
    with pytest.raises(ValueError, match="Too many Z gates in parallel"):
        d.validate_operation(cirq.ParallelGate(cirq.Z, len(d.qubits)).on(*d.qubits))
    with pytest.raises(ValueError, match="Bad number of XY gates in parallel"):
        d.validate_operation(
            cirq.ParallelGate(cirq.X, len(d.qubit_list()[1:])).on(*d.qubit_list()[1:])
        )
...
def test_validate_circuit_errors():
    ...
    q11 = cirq.GridQubit(1, 1)
    c = cirq.Circuit()
    c.append(cirq.ParallelGate(cirq.X, len(d.qubits)).on(*d.qubits))
    c.append(cirq.CCZ.on(q00, q01, q10))
    c.append(cirq.ParallelGate(cirq.Z, 3).on(*[q00, q01, q10]))
