...
def merge_single_qubit_gates_to_phxz_symbolized(
    ...
    *,
    ...
    """Merge consecutive single qubit gates as PhasedXZ Gates. Symbolized if any of the consecutive gates is symbolized.
    ...
    # Stores the number of merges for all set of resolvers,
    ...
    output_circuit = transformer_primitives.map_operations(
        merged_circuits[0].freeze(), _map_func, deep=deep
    )
    ...
    for merged_circuit in merged_circuits:
        for op in merged_circuit.all_operations():
            ...
            if len(the_merge_tag) > 1:
                raise RuntimeError("Multiple merge tags found.")
            sid = the_merge_tag.pop().split("_")[-1]
            x, z, a = 0, 0, 0  # Identity gate's parameters.
            if isinstance(op.gate, ops.PhasedXZGate):
    ...
    return output_circuit, new_sweep
