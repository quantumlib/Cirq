...
def quantum_shannon_decomposition(qubits : list , U : np.ndarray, ops = None):
    ...
    if not (n and not (n & (n-1)) and n == m):
        raise ValueError(f"Expected input matrix U to a (2^n x 2^n) shaped numpy array, but instead got shape {(n,m)}")
        
    if not cirq.is_unitary(U):     # Check that U is unitary
        raise ValueError(f"Expected input matrix U to be unitary, but it fails cirq.is_unitary check")
