...
def _decompose_single_ctrl(
    matrix: np.ndarray, control: ops.Qid, target: ops.Qid
) -> list[ops.Operation]:
...
def _ccnot_congruent(c0: ops.Qid, c1: ops.Qid, target: ops.Qid) -> list[ops.Operation]:
...
def decompose_multi_controlled_x(
    controls: list[ops.Qid], target: ops.Qid, free_qubits: list[ops.Qid]
) -> list[ops.Operation]:
...
def _decompose_su(
    matrix: np.ndarray, controls: list[ops.Qid], target: ops.Qid
) -> list[ops.Operation]:
...
def _decompose_recursive(
    matrix: np.ndarray,
    power: float,
    controls: list[ops.Qid],
    target: ops.Qid,
    free_qubits: list[ops.Qid],
) -> list[ops.Operation]:
