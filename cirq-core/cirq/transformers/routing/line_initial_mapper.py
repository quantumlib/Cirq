...
class LineInitialMapper(routing.AbstractInitialMapper):
    ...
    def _make_circuit_graph(self) -> nx.Graph:
        ...
        for op in self.circuit.all_operations():
            ...
            node_order += 1
            if len(op.qubits) == 2 and all(circuit_graph.degree[op.qubits[i]] < 2 for i in range(2)):
        ...
        while found:
            try:
                ...
                edge_to_remove = max(cycle, key= lambda x: circuit_graph.edges[x[0], x[1]]['edge_order'])
