...
def test_mapping_manager():
    ...
    mm.inverse_map == {v:k for k,v in mm.map.items()}

    # apply same swap and test shortest path for a couple pairs
    mm.apply_swap(q[3], q[2])
    assert mm.shortest_path(q[1], q[2]) == [cirq.NamedQubit("a"), cirq.NamedQubit("b"), cirq.NamedQubit("c")]
    ...
    shortest_one_to_four = [cirq.NamedQubit("a"), cirq.NamedQubit("b"), cirq.NamedQubit("c"), cirq.NamedQubit("d")]
