...
def test_distance_on_device_and_can_execute():
    ...
    assert mm.can_execute(q_int[1], q_int[3]) is False
    assert mm.dist_on_device(q_int[1], q_int[2]) == 1
    assert mm.can_execute(q_int[1], q_int[2])
