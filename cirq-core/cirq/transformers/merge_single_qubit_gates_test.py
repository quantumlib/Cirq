...
def test_merge_single_qubit_moments_to_phxz():
    ...
    c_orig = cirq.Circuit(
        ...
        cirq.Moment(cirq.Z.on_each(*q[1:])),
        cirq.Moment(cirq.T.on_each(*q[:2])),
...
''',
    ...
    cirq.testing.assert_has_diagram(
        c_new,
        '''
0: ───PhXZ(a=1.125,x=1,z=0)───@───PhXZ(a=1.125,x=1,z=0)───Y['nocompile']───X───M───────────
                              │                                                ║
1: ───PhXZ(a=0.625,x=1,z=0)───@───PhXZ(a=0.625,x=1,z=0)───Z────────────────────╫───X───X───
                                                                               ║   ║
2: ───PhXZ(a=-0.5,x=0,z=-1)───Y───PhXZ(a=-0.5,x=0,z=-1)───Z────────────────────╫───╫───────
                                                                               ║   ║
a: ════════════════════════════════════════════════════════════════════════════@═══^═══════
''',
        precision=4,
