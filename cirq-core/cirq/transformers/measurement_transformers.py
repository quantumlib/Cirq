...
class _ConfusionChannel(ops.Gate):
    ...
    def _apply_channel_(self, args: 'cirq.ApplyChannelArgs'):
        configs: List['cirq.BuildFromSlicesArgs'] = []
        for i in range(np.prod(self._shape) ** 2):
            ...
            slices: List['cirq.SliceConfig'] = []
            axis_count = len(args.left_axes)
            for j in range(axis_count):
                s1 = transformations.SliceConfig(
                ...
                slices.extend([s1, s2])
            configs.append(transformations.BuildFromSlicesArgs(slices=tuple(slices), scale=scale))
