...
def _numpy_arrays_to_state_vectors_or_density_matrices(
    ...
    if state1.ndim > 2 or (state1.ndim == 2 and state1.shape[0] != state1.shape[1]):
        ...
        state1 = np.reshape(state1, (np.prod(state1.shape, dtype=int).item(),))
    if state2.ndim > 2 or (state2.ndim == 2 and state2.shape[0] != state2.shape[1]):
        # State tensor, convert to state vector
        state2 = np.reshape(state2, (np.prod(state2.shape, dtype=int).item(),))
    if state1.ndim == 2 and state2.ndim == 2:
        ...
        if state1.shape == state2.shape:
            ...
            if state1.shape == qid_shape:
                ...
                state1 = np.reshape(state1, (np.prod(qid_shape, dtype=int).item(),))
                state2 = np.reshape(state2, (np.prod(qid_shape, dtype=int).item(),))
        ...
        state1.ndim == 2
        and state2.ndim < 2
        and np.prod(state1.shape, dtype=int) == np.prod(state2.shape, dtype=int)
    ):
    ...
    if validate:
        dim1: int = state1.shape[0] if state1.ndim == 2 else np.prod(state1.shape, dtype=int).item()
        dim2: int = state2.shape[0] if state2.ndim == 2 else np.prod(state2.shape, dtype=int).item()
        if dim1 != dim2:
            raise ValueError('Mismatched dimensions in given states: ' f'{dim1} and {dim2}.')
        if qid_shape is None:
            qid_shape = (dim1,)
        else:
            expected_dim = np.prod(qid_shape, dtype=int)
