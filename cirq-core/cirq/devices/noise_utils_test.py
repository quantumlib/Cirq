...
def test_op_identifier():
    ...
    op_id = OpIdentifier(cirq.CZPowGate, q0, q1)
    assert str(op_id) == 'OpID(CZPowGate(cirq.LineQubit(0), cirq.LineQubit(1)))'
    assert repr(op_id) == (
        'cirq.devices.noise_utils.OpIdentifier('
        'cirq.ops.common_gates.CZPowGate, '
        'cirq.LineQubit(0), cirq.LineQubit(1))'
    )
    assert op_id.swapped() == OpIdentifier(cirq.CZPowGate, q1, q0)
