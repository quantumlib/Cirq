...
class TestSimulator():
  ...
  def test_x_gate():
    ...
    circuit =  cirq.Circuit(cirq.X(q0), cirq.measure(cirq.LineQubit(q0), key='k'))
    expected_results ={'key': np.array[[[1]]]}
    simulator = ClassicalSimulator()
    assert expect_result == classical_simulator.run(circuit)
  ...
  def test_x_gate():
    ...
    circuit =  cirq.Circuit(cirq.X(q0), cirq.measure(cirq.LineQubit(q0), key='k'))
    expected_results = {'key': np.array[[[0]]]}
...
def test_CNOT():
  q0, q1 = cirq.LineQubit.range(2)
  circuit  = cirq.Circuit()
  circuit =  cirq.Circuit(cirq.X(q0), cirq.CNOT(q0, q1), cirq.measure(cirq.LineQubit(q1), key='k'))
  expected_results = {'key': np.array[[[1]]]}
  results = run(circuit = circuit, param_resolver=None, repetitions = 1 )
  assert results == expected_results
