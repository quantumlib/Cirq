...
from cirq.sim import ClassicalStateSimulator

#TODO: reforctor test to match custom simulators test
#TODO: methods for construcitng simulator with/without paramters
#TODO: applying in/valid gates
#TODO: in/valid classical simulator 

def test_x_gate(self):
    q0, q1 = cirq.LineQubit.range(2)
    sim = CustomStateSimulator(ClassicalStateSimulator)
    circuit = cirq.Circuit()
    circuit.append(cirq.X(q0))
    circuit.append(cirq.X(q1))
    circuit.append(cirq.X(q1))
    circuit.append(cirq.measure(q0, key='a'))
    circuit.append(cirq.measure(q1, key='b'))
    r = sim.simulate(circuit)
    assert r.measurements == {'a': np.array([1]), 'b': np.array([0])}
    assert r._final_simulator_state._state.basis == [1, 0]
