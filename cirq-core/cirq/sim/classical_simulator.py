    ...
    def _run( circuit: 'cirq.AbstactCircuit', param_resolver: 'cirq.ParamResolver',
     ...
     for moment in resolved_circuit:
        for op in moment: 
            ...
            ##add the new instance of a key to the numpy array in results dictionary
                if(gate.key) in results_dict.keys():
                    ...
                    for bits in range(0, len(qubits_in_order)):
                        ...
                        results_dict[gate.key] = np.insert(current_array, 1, new_instance, axis=1)
                else:
           ...
           elif not((isinstance(gate, cirq.XPowGate) and gate.exponent == 0) or (isinstance(gate, cirq.CNotPowGate) and gate.exponent == 0)):
