...
def test_init_state():
    args = cirq.CHFormSimulationState(qubits=cirq.LineQubit.range(1), initial_state=1)
    np.testing.assert_allclose(args.state.state_vector(), [0, 1])
    with pytest.raises(ValueError, match='Must specify qubits'):
        _ = cirq.CHFormSimulationState(initial_state=1)
...
def test_cannot_act():
    ...
    args = cirq.CHFormSimulationState(qubits=[], prng=np.random.RandomState())
...
def test_gate_with_act_on():
    class CustomGate(cirq.testing.SingleQubitGate):
        def _act_on_(self, args, qubits):
            if isinstance(args, cirq.CHFormSimulationState):
