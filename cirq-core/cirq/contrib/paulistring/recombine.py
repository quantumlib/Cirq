...
) -> List[Tuple[ops.PauliStringPhasor, int, Unique[ops.PauliStringPhasor]]]:
...
def move_pauli_strings_into_circuit(
    circuit_left: Union[circuits.Circuit, CircuitDag], circuit_right: circuits.Circuit
) -> circuits.Circuit:
    if isinstance(circuit_left, CircuitDag):
        string_dag = CircuitDag(pauli_string_reorder_pred, circuit_left)
    else:
