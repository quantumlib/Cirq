...
def _cirq_to_circuits_and_ops(
    ...
    def _flush(cc: zx.Circuit) -> zx.Circuit:
        """If `cc` holds any PyZX gates, append it and return a fresh empty circuit."""
        if cc.gates:
            circuits_and_ops.append(cc)
            return zx.Circuit(len(qubits), bit_amount=bit_amount)
        return cc

    for op in circuit.all_operations():
        if tags_to_ignore.intersection(op.tags):
            current_circuit = _flush(current_circuit)
