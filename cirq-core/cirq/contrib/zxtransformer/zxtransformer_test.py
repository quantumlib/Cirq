...
from cirq.contrib.zxtransformer.zxtransformer import (
    _cirq_gate_to_zx_gate,
    _cirq_to_circuits_and_ops,
    _make_cirq_gate,
    _recover_circuit,
    zx_transformer,
)
...
def _run_zxtransformer(
    qc: cirq.Circuit, optimizer: Callable[[zx.Circuit], zx.Circuit] | None = None
) -> None:
...
def test_measurement_converted_to_pyzx() -> None:
    ...
    circuits_and_ops, keys, _, _ = _cirq_to_circuits_and_ops(circuit, [*circuit.all_qubits()])
...
def test_reset_converted_to_pyzx() -> None:
    ...
    circuits_and_ops, _, _, _ = _cirq_to_circuits_and_ops(circuit, [*circuit.all_qubits()])
    assert len(circuits_and_ops) == 1
    assert isinstance(circuits_and_ops[0], zx.Circuit)
    pyzx_circuit = circuits_and_ops[0]
...
def test_separate_same_key_measurements_not_merged() -> None:
    """Two separate same-key measurements on different qubits must not be merged.

    Cirq treats repeated measurement keys as accumulating separate result records,
    so `measure(q0, key='m'), measure(q1, key='m')` must remain two ops after the
    transformer round-trip, not collapse into `measure(q0, q1, key='m')`.
    """
    q = cirq.LineQubit.range(2)
    circuit = cirq.Circuit(
        cirq.X(q[0]), cirq.X(q[1]), cirq.measure(q[0], key='m'), cirq.measure(q[1], key='m')
    )
    result = zx_transformer(circuit)
    measurement_ops = [op for moment in result for op in moment if cirq.is_measurement(op)]
    assert len(measurement_ops) == 2
    for op in measurement_ops:
        assert cirq.measurement_key_name(op) == 'm'
        assert len(op.qubits) == 1


def test_measurement_with_confusion_map_passthrough() -> None:
    """A measurement carrying a `confusion_map` must be preserved untouched.

    The confusion map models classical readout error, which PyZX cannot represent, so
    the operation must be emitted opaquely rather than reconstructed without the map.
    """
    q = cirq.NamedQubit("q")
    confusion = np.array([[0.9, 0.1], [0.1, 0.9]])
    gate = cirq.MeasurementGate(1, 'm', confusion_map={(0,): confusion})
    circuit = cirq.Circuit(cirq.X(q), gate.on(q))
    result = zx_transformer(circuit)
    measurement_ops = [op for moment in result for op in moment if cirq.is_measurement(op)]
    assert len(measurement_ops) == 1
    out_gate = measurement_ops[0].gate
    assert isinstance(out_gate, cirq.MeasurementGate)
    assert (0,) in out_gate.confusion_map
    np.testing.assert_array_equal(out_gate.confusion_map[(0,)], confusion)


def test_parameterized_gate_passthrough() -> None:
    """Parameterized (symbolic) gates cannot be converted to PyZX rationals and must pass through
    as opaque `cirq.Operation`s rather than crash."""
    q = cirq.NamedQubit("q")
    theta = sympy.Symbol('theta')
    circuit = cirq.Circuit(cirq.H(q), cirq.XPowGate(exponent=theta)(q), cirq.H(q))
    # Should not raise.
    result = zx_transformer(circuit)
    # The parameterized gate must be preserved as-is somewhere in the output.
    param_ops = [op for moment in result for op in moment if cirq.is_parameterized(op)]
    assert len(param_ops) == 1


def test_parameterized_conditional_gate_passthrough() -> None:
    """A classically controlled parameterized gate must pass through as an opaque op."""
    q = cirq.NamedQubit("q")
    theta = sympy.Symbol('theta')
    circuit = cirq.Circuit(cirq.XPowGate(exponent=theta)(q).with_classical_controls('c'))
    # Should not raise.
    result = zx_transformer(circuit)
    cond_ops = [
        op
...
def test_qubit_ordering_is_deterministic() -> None:
    """PyZX line indices must be stable across runs, so `all_qubits()` iteration must be sorted.

    `NamedQubit`s hash on string, and Python randomizes string hashing per process, so a
    frozenset traversal order is unstable.  Feeding that order to PyZX would produce
    different optimized circuits from run to run for the exact same input.
    """
    q = [cirq.NamedQubit(name) for name in ('charlie', 'alpha', 'bravo')]
    circuit = cirq.Circuit(cirq.H(q[0]), cirq.CX(q[0], q[1]), cirq.CX(q[1], q[2]))
    result_a = zx_transformer(circuit)
    result_b = zx_transformer(circuit)
    assert result_a == result_b


def test_tags_to_ignore_passes_op_through_untouched() -> None:
    """Operations carrying a tag in `context.tags_to_ignore` must survive unchanged."""
    q = cirq.LineQubit.range(2)
    # H H would normally cancel under `full_reduce`; the tag must prevent that.
    tagged_h = cirq.H(q[0]).with_tags('no_optimize')
    circuit = cirq.Circuit(tagged_h, cirq.H(q[0]), cirq.CX(q[0], q[1]))
    context = cirq.TransformerContext(tags_to_ignore=('no_optimize',))
    result = zx_transformer(circuit, context=context)
    tagged_ops = [op for moment in result for op in moment if 'no_optimize' in op.tags]
    assert len(tagged_ops) == 1
    assert tagged_ops[0].gate == cirq.H
    assert tagged_ops[0].qubits == (q[0],)


def test_tags_to_ignore_preserves_tagged_measurement() -> None:
    """A tagged measurement must not be rewritten by the reverse mapping."""
    q = cirq.NamedQubit('q')
    circuit = cirq.Circuit(cirq.X(q), cirq.measure(q, key='m').with_tags('protect'))
    context = cirq.TransformerContext(tags_to_ignore=('protect',))
    result = zx_transformer(circuit, context=context)
    measurement_ops = [op for moment in result for op in moment if cirq.is_measurement(op)]
    assert len(measurement_ops) == 1
    assert 'protect' in measurement_ops[0].tags
...
def test_cirq_gate_to_zx_gate_none_gate() -> None:
    """`_cirq_gate_to_zx_gate` returns None when given a None gate."""
    assert _cirq_gate_to_zx_gate(None, [0]) is None


def test_cirq_gate_to_zx_gate_unsupported_gate() -> None:
    """`_cirq_gate_to_zx_gate` returns None for a gate not in the forward mapping."""
    assert _cirq_gate_to_zx_gate(cirq.ISWAP, [0, 1]) is None


def test_make_cirq_gate_unknown_name() -> None:
    """`_make_cirq_gate` raises ValueError for an unsupported gate name."""
    with pytest.raises(ValueError, match="Unsupported gate"):
        _make_cirq_gate("not_a_real_gate")


def test_unsupported_gate_passthrough_in_circuit() -> None:
    """A circuit containing an unsupported gate keeps that gate as an opaque cirq.Operation."""
    q = cirq.LineQubit.range(2)
    circuit = cirq.Circuit(cirq.H(q[0]), cirq.ISWAP(*q), cirq.H(q[1]))
    circuits_and_ops, _, _, _ = _cirq_to_circuits_and_ops(circuit, [*circuit.all_qubits()])
    # Expected: pyzx Circuit(H), opaque ISWAP, pyzx Circuit(H).
    assert len(circuits_and_ops) == 3
    assert isinstance(circuits_and_ops[0], zx.Circuit)
    assert isinstance(circuits_and_ops[1], cirq.Operation)
    assert circuits_and_ops[1].gate == cirq.ISWAP
    assert isinstance(circuits_and_ops[2], zx.Circuit)

    # The transformer round-trip should preserve the unsupported gate.
    result = zx_transformer(circuit)
    iswap_ops = [op for moment in result for op in moment if op.gate == cirq.ISWAP]
    assert len(iswap_ops) == 1


def test_conditional_gate_multiple_controls_passthrough() -> None:
    """A classically controlled op with multiple controls is passed through opaquely."""
    q = cirq.NamedQubit("q")
    circuit = cirq.Circuit(cirq.X(q).with_classical_controls('a', 'b'))
    circuits_and_ops, _, _, _ = _cirq_to_circuits_and_ops(circuit, [*circuit.all_qubits()])
    assert len(circuits_and_ops) == 1
    assert isinstance(circuits_and_ops[0], cirq.ClassicallyControlledOperation)


def test_conditional_gate_sympy_condition_passthrough() -> None:
    """A classically controlled op with a non-`KeyCondition` is passed through opaquely."""
    q = cirq.NamedQubit("q")
    circuit = cirq.Circuit(cirq.X(q).with_classical_controls(sympy.Symbol('a') > 0))
    circuits_and_ops, _, _, _ = _cirq_to_circuits_and_ops(circuit, [*circuit.all_qubits()])
    assert len(circuits_and_ops) == 1
    assert isinstance(circuits_and_ops[0], cirq.ClassicallyControlledOperation)


def test_conditional_gate_multi_qubit_passthrough() -> None:
    """A classically controlled multi-qubit op is passed through opaquely."""
    q = cirq.LineQubit.range(2)
    circuit = cirq.Circuit(cirq.CNOT(*q).with_classical_controls('c'))
    circuits_and_ops, _, _, _ = _cirq_to_circuits_and_ops(circuit, [*circuit.all_qubits()])
    assert len(circuits_and_ops) == 1
    assert isinstance(circuits_and_ops[0], cirq.ClassicallyControlledOperation)


def test_recover_circuit_missing_result_bit() -> None:
    """`_recover_circuit` raises ValueError if a PyzxMeasurement has no result_bit."""
    q = cirq.NamedQubit("q")
    pyzx_circuit = zx.Circuit(1, bit_amount=1)
    pyzx_circuit.add_gate(PyzxMeasurement(0, result_bit=None))
    with pytest.raises(ValueError, match="missing result_bit"):
        _recover_circuit(
            [pyzx_circuit],
            [q],
            measurement_keys=['m'],
            measurement_invert=[False],
            measurement_op_ids=[0],
        )


def test_recover_circuit_out_of_range_result_bit() -> None:
    """`_recover_circuit` raises ValueError if a PyzxMeasurement result_bit is out of range."""
    q = cirq.NamedQubit("q")
    pyzx_circuit = zx.Circuit(1, bit_amount=1)
    pyzx_circuit.add_gate(PyzxMeasurement(0, result_bit=5))
