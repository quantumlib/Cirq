...
def test_measurement_key():
    a = cirq.NamedQubit('a')
    assert cirq.measurement_key_str(cirq.measure(a, key='lock')) == 'lock'
...
def test_with_measurement_key_mapping():
    ...
    assert cirq.measurement_keys_str(remap_op) == {'k'}
...
def test_with_key_path():
    ...
    assert cirq.measurement_keys_str(remap_op) == {'a:b:m'}
