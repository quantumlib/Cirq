...
def test_kron_spreads_values():
    ...
    assert np.allclose(
        cirq.kron(np.eye(2), u), np.array([[2, 3, 0, 0], [5, 7, 0, 0], [0, 0, 2, 3], [0, 0, 5, 7]])
    )

    assert np.allclose(
        cirq.kron(u, np.eye(2)), np.array([[2, 0, 3, 0], [0, 2, 0, 3], [5, 0, 7, 0], [0, 5, 0, 7]])
    )
...
def test_block_diag():
    ...
    assert np.allclose(
        cirq.block_diag(np.array([[1, 2], [3, 4]]), np.array([[4, 5, 6], [7, 8, 9], [10, 11, 12]])),
        np.array(
...
def test_block_diag_dtype():
    ...
    assert (
        cirq.block_diag(np.array([[1]], dtype=np.float32), np.array([[2]], dtype=np.float32)).dtype
        == np.float32
    )

    assert (
        cirq.block_diag(np.array([[1]], dtype=np.float64), np.array([[2]], dtype=np.float64)).dtype
        == np.float64
    )

    assert (
        cirq.block_diag(np.array([[1]], dtype=np.float32), np.array([[2]], dtype=np.float64)).dtype
        == np.float64
    )

    assert (
        cirq.block_diag(
            np.array([[1]], dtype=np.float32), np.array([[2]], dtype=np.complex64)
        ).dtype
        == np.complex64
    )
