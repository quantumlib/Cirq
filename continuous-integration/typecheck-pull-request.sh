...
# runs mypy on it. It informs github of the outcome of the linting via the
# 'typecheck (manual)' status indicator.
#
#   bash typecheck-pull-request.sh [pull_request_number] [access_token]
#
...
github_context="typecheck by maintainer"
...
set +e
mypy --config-file "${work_dir}/continuous-integration/mypy.ini" cirq
outcome=$?
set -e
deactivate
