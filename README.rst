...
Note: The (*default*) documentation for ``stable`` might be behind ``latest`` (see `here (https://cirq.readthedocs.io/en/latest/tutorial.html`_), which can cause confusion if you are working on branches such as ``cirq-dev`` or ``cirq-release``.
